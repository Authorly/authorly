...
if ENV['RACK_ENV'] == 'production'
  set :cookie_options, :expires => Time.now + 365 * 24 * 60 * 60, :domain => '.authorly.com' # expire 1 year from now
elsif ENV['RACK_ENV'] == 'development'
  set :cookie_options, :expires => Time.now + 365 * 24 * 60 * 60, :domain => '.interapptive.com' # expire 1 year from now
end

get '/' do
  logger.info "params: #{params.inspect}"

  km_id = cookies[:km_id].nil? ? cookies[:email] : cookies[:km_id]
  logger.info km_id
